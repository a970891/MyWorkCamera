...
- (void)initQrMenu {
    ...
    CGFloat height = 100*AUTO_HEIGHT;
//    CGFloat width = self.bounds.size.width;
    qrMenu = [[QRMenu alloc] initWithFrame:CGRectMake(60*AUTO_WIDTH, self.bounds.size.height - height*1.5, 200*AUTO_WIDTH , height)];
...
- (void)show {
    ...
    [UIView animateWithDuration:kQrLineanimateDuration animations:^{
        ...
        CGFloat maxBorder = (self.frame.size.height-63.5*AUTO_HEIGHT)/ 2 + self.transparentArea.height / 2 - 4;
        if (qrLineY > maxBorder) {
            
            qrLineY = (self.frame.size.height-63.5*AUTO_HEIGHT) / 2 - self.transparentArea.height /2;
        }
        qrLineY++;
    }];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    CGRect screenDrawRect =CGRectMake(0, 63.5*AUTO_HEIGHT, screenSize.width,screenSize.height-63.5*AUTO_HEIGHT);
